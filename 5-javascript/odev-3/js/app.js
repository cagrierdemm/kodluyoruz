...
const itemSection = document.querySelector(".section-center.row");

const categories = menu.reduce((i,v) => {
  if(!i.includes(v.category)){
    i.push(v.category)
  }
  return i;
}, ["All"]);

const categoriesButtons = categories.map(categories => {
  return `<button class="btn btn-outline-dark btn-item" data-id="${categories}">${categories}</button>`
}).join("");

btnSection.innerHTML=categoriesButtons;
...
function myFunction(menu){
  ...
  divMenuText.innerHTML=`${menu.desc}`
...
buttons.forEach(btn =>{
  btn.addEventListener("click", e => {
    ...
    let filteredMenu = x=="All" ? menu : menu.filter(item => item.category === x);
    console.log(filteredMenu)
    itemSection.innerHTML = "";
    filteredMenu.forEach(myFunction);
  })
})
